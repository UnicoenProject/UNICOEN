...
namespace Unicoen.Languages.JavaScript.ProgramGenerators {
	public static class JavaScriptProgramGeneratorHelper {
		...
		public static UnifiedFunctionDefinition CreateFunctionDeclaration(
			...
			var parameters =
					CreateFormalParameterList(
							node.Element("formalParameterList"));
		...
		public static UnifiedVariableDefinitionList
			...
			var declarators =
					node.Elements("variableDeclaration").Select(
		...
		public static UnifiedVariableDefinition
			...
			var name = UnifiedVariableIdentifier.Create(
					node.NthElement(0).Value);
			var init = node.Element("initialiser") != null
							   ? CreateInitialiser(node.Element("initialiser"))
							   : null;
		...
		public static UnifiedVariableDefinition
			...
			var init = node.Element("initialiserNoIn") != null
							   ? CreateInitialiserNoIn(
									   node.Element("initialiserNoIn")) : null;
		...
		public static UnifiedCaseCollection CreateCaseBlock(XElement node) {
			...
			foreach (
					var e in
							node.Elements().Where(
									e => e.Name().EndsWith("Clause"))
					) {
				cases.Add(
						e.Name() == "caseClause"
								? CreateCaseClause(e) : CreateDefaultClause(e));
			}
			return cases;
		}
			...
			case "leftHandSideExpression":
				return UnifiedBinaryExpression.Create(
						...
						CreateAssignmentOperator(
								node.Element("assignmentOperator")),
						CreateAssignmentExpression(
								node.Element("assignmentExpression")));
			default:
				throw new InvalidOperationException();
			...
			if (node.HasElement("assignmentExpression")) {
				return UnifiedTernaryExpression.Create(
						CreateLogicalORExpression(
								node.Element("logicalORExpression")),
						CreateAssignmentExpression(
								node.Element("assignmentExpression")),
						CreateAssignmentExpression(
								node.Elements("assignmentExpression").ElementAt(
										1)));
			}
		...
		public static UnifiedExpression CreatePostfixExpression(XElement node) {
			...
			if (node.Elements().Count() == 2) {
				...
				return
						UnifiedUnaryExpression.Create(
								CreateLeftHandSideExpression(node.NthElement(0)),
								ope);
			}
			return CreateLeftHandSideExpression(node.NthElement(0));
		}
		...
		public static UnifiedLiteral CreateRegularExpressionLiteral(
				XElement node) {
			Contract.Requires(node != null);
		...
		private static UnifiedUnaryOperator CreatePrefixUnaryOperator(
				string name) {

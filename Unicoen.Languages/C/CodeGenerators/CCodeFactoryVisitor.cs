...
namespace Unicoen.Languages.C.CodeGenerators {
    public partial class CCodeFactoryVisitor :
        ...
        protected static readonly Decoration ForTopBlock = new Decoration
        { EachRight = "\n" };

        // ブロック構文の開始と終了を示す中括弧とブロック内の各要素を区切る改行記号
        protected static readonly Decoration ForBlock = new Decoration
        { MostLeft = "{", EachRight = "\n", MostRight = "}" };

        // パラメータや実引数を表現するために使用される
        protected static readonly Decoration Paren = new Decoration
        { MostLeft = "(", Delimiter = ", ", MostRight = ")" };

        // 変数宣言を1行に書く際に使用される e.g. int a, b;
        protected static readonly Decoration CommaDelimiter = new Decoration
        { Delimiter = ", " };

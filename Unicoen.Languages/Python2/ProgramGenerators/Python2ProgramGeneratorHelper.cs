...
namespace Unicoen.Languages.Python2.ProgramGenerators {
	public static class Python2ProgramGeneratorHelper {
		...
		public static UnifiedProgram CreateFile_input(XElement node) {
			Contract.Requires(node != null);
			Contract.Requires(
					node.Name() == "file_input"
					|| node.Name() == "encoding_decl");
			/*
		...
		public static UnifiedAnnotation CreateDecorator(XElement node) {
			...
			return
					UnifiedAnnotation.Create(
							CreateDotted_name(node.NthElement(1)).ToProperty(
									"."), arglist
							);
		}

		public static UnifiedSet<UnifiedAnnotation> CreateDecorators(
				XElement node) {
			Contract.Requires(node != null);
			 ...
			 */
			return node.Elements().Select(CreateDecorator).ToSet();
		}
		...
		public static UnifiedFunctionDefinition CreateFuncdef(XElement node) {
			...
			return UnifiedFunctionDefinition.Create(
					...
					CreateParameters(node.NthElement(2)), null,
					CreateSuite(node.LastElement()));
		}

		public static UnifiedSet<UnifiedParameter> CreateParameters(XElement node) {
			Contract.Requires(node != null);
			...
			return varargslist != null
						   ...
						   : UnifiedSet<UnifiedParameter>.Create();
		}

		public static UnifiedSet<UnifiedParameter> CreateVarargslist(
				XElement node) {
			Contract.Requires(node != null);
							...
							e => {
								...
								if (next == null || next.Value != "=") {
									return UnifiedParameter.Create(
											null,
											null, null,
											names.Select(UnifiedVariableIdentifier.Create).ToSet<UnifiedIdentifier>());
								}
								return UnifiedParameter.Create(
										null,
										null, null,
		...
		public static IEnumerable<UnifiedImport> CreateImport_stmt(
				XElement node) {
			Contract.Requires(node != null);
		...
		public static IEnumerable<UnifiedImport> CreateImport_name(
				XElement node) {
			Contract.Requires(node != null);
		...
		public static Tuple<UnifiedVariableIdentifier, string>
				CreateImport_as_name(
				XElement node) {
			Contract.Requires(node != null);
		...
		public static IEnumerable<UnifiedIdentifier> CreateDotted_name(
				XElement node) {
			Contract.Requires(node != null);
		...
		public static UnifiedExpression CreateGlobal_stmt(XElement node) {
			...
			return node.Elements("NAME").Select(
					e => UnifiedVariableDefinition.Create(
							modifiers:
									UnifiedModifier.Create("global").
									ToSet(),
							name: e.Value.ToVariableIdentifier())).
		...
		public static UnifiedIf CreateIf_stmt(XElement node) {
			...
			var elseSuite = elseSuiteNode.PreviousElements().ElementAt(1).Value
							== "else"
			...
			var catches = exceptClauseNodes
					...
					.Select(
							t => UnifiedCatch.Create(
									UnifiedType.Create(t.Item1.Item1).
											ToSet(),
									t.Item1.Item2,
									t.Item2))
					.ToSet();
		...
		public static UnifiedExpression CreateListmaker(XElement node) {
			...
			return UnifiedListComprehension.Create(
					...
					CreateList_for(list_forNode).ToSet());
		}
		...
		public static UnifiedExpression CreateTestlist_comp(XElement node) {
			...
			return UnifiedIterableComprehension.Create(
					CreateTest(node.FirstElement()),
					CreateComp_for(comp_forNode).ToSet());
		}
		...
		public static UnifiedSet<UnifiedArgument> CreateSubscriptlist(
				XElement node) {
			Contract.Requires(node != null);
		...
		public static UnifiedArgument CreateArgument(XElement node) {
			...
			if (second.Value == "=") {
				...
				return UnifiedArgument.Create(
						CreateTest(node.LastElement()), (UnifiedIdentifier)test,
						null);
			}
			return UnifiedArgument.Create(
					UnifiedIterableComprehension.Create(
							test,
							CreateComp_for(second).ToSet()), null, null);
		}

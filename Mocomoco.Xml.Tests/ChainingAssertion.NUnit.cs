...
namespace NUnit.Framework {
	...
	public static class AssertEx {
		...
		public static void Is<T>(
				this T value, Expression<Func<T, bool>> predicate,
				string message = "") {
		...
		private class DynamicAccessor<T> : DynamicObject {
			...
			public override bool TrySetIndex(
					SetIndexBinder binder, object[] indexes,
					object value) {
				try {
					typeof(T).InvokeMember(
							"Item", TransparentFlags | BindingFlags.SetProperty,
			...
			private MethodInfo MatchMethod(
								...
								mi => {
									...
									if (!typeArgs.Any() && !genericArguments.Any()) // non generic method
									{
										...
										var typeParams = genericArguments
												.GroupJoin(
														parameterGenericTypes, x => x, x => x.Key,
														(_, Args) => Args)
												.ToArray();
				...
				if (!typedMethods.Any())
					throw new ArgumentException(
							string.Format(
									"\"{0}\" not match arguments : Type <{1}>", methodName,
									typeof(T).Name));
				...
				throw new ArgumentException(
						string.Format(
								"\"{0}\" ambiguous arguments : Type <{1}>", methodName,
								typeof(T).Name));
			}

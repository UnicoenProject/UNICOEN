...
namespace Ucpf.Languages.C.Tests {
	...
	public class CCodeModelTestForFibonacci {
		...
		public void ElseBlockが正しく生成できる() {
			var firstStatemenet =
				((CIfStatement)(_function.Body.Statements.ElementAt(0)))
					.ElseBlock
					.Statements
					.ElementAt(0)
				as CReturnStatement;
			...
			var leftArg =
				(CBinaryExpression)
				((CInvocationExpression)leftExpression).Arguments.ElementAt(0);
			...
			var rightArg =
				(CBinaryExpression)
				((CInvocationExpression)rightExpression).Arguments.ElementAt(0);

			Assert.That(rightFuncName, Is.EqualTo("fibonacci"));
			Assert.That(rightArg.LeftExpression.ToString(), Is.EqualTo("n"));
			Assert.That(rightArg.Operator.Type,
				Is.EqualTo(BinaryOperatorType.Subtraction));
		...
		public void TrueBlockが正しく生成できる() {
			...
			Assert.That(firstStatement.Expression.ToString(), Is.EqualTo("n"));
			// assert body
		}

...
namespace Unicoen.Apps.UniAspect.Cui.Processor.Pointcut {
    ...
    public class Execution : CodeProcessor {
        ...
        public static void InsertAtBeforeExecution(
            ...
            foreach (var function in functions) {
                ...
                if (m.Success) {
                    ...
                    var copy = ReplaceSpecialToken(
                            advice.DeepCopy(), function.Name.Name);
                    //アドバイスを対象関数に合成する
        ...
        /// </summary>
        public static UnifiedBlock ReplaceSpecialToken(
                UnifiedBlock old, string functionName) {
            ...
            foreach (var function in functions) {
                ...
                if (returns.Count() == 0) {
                    ...
                    foreach (var returnStmt in returns) {
                        ...
                        block.Insert(
                                block.IndexOf(returnStmt, 0), advice.DeepCopy());
                    }
                }
            }
        }
        ...
        /// <param name="advice"> 挿入するコード断片 </param>
        public static void InsertAtBeforeExecution(
                IUnifiedElement root, Regex regex, int statementNum,
                UnifiedBlock advice) {
            //関数の一覧を取得
            var functions = root.Descendants<UnifiedFunctionDefinition>();
            ...
            foreach (var function in functions) {
                ...
                var innerStatements =
                        function.Body.Descendants().Where(
                                e =>
                                e.Parent.GetType().Equals(typeof(UnifiedBlock)));
        ...
        /// <param name="advice"> 挿入するコード断片 </param>
        public static void InsertAtBeforeExecution(
                IUnifiedElement root, Regex regex, Type element,
                UnifiedBlock advice) {
            //関数の一覧を取得
            var functions = root.Descendants<UnifiedFunctionDefinition>();
        ...
        /// <param name="advice"> 挿入するコード断片 </param>
        public static void InsertAtBeforeExecutionByName(
                IUnifiedElement root, string name, int statementNum,
                UnifiedBlock advice) {
        ...
        /// <param name="advice"> 挿入するコード断片 </param>
        public static void InsertAtBeforeExecutionByName(
                IUnifiedElement root, string name, Type element,
                UnifiedBlock advice) {
            InsertAtBeforeExecution(
                    root, new Regex("^" + name + "$"), element, advice);
        }

...
namespace Unicoen.Apps.UniAspect.Cui.Processor {
    public class UcoGenerator {
        ...
        public static UnifiedBlock CreateAdvice(string language, string code) {
            ...
            case "JavaScript":
                code = "{ " + code + " }";
                ast = JavaScriptCodeToXml.Instance.Generate(
                        code, p => p.statementBlock());
                actual =
            ...
            case "C":
                code = "{ " + code + " }";
                ast = CCodeToXml.Instance.Generate(
                        code, p => p.compound_statement());
        ...
        private static UnifiedBlock CreateAdviceForCSharp(string code) {
            ...
            var block =
                    model.Descendants<UnifiedFunctionDefinition>().First().Body;
            return block;
        }
        ...
        public static List<IUnifiedExpression> CreateIntertype(
            ...
            case "JavaScript":
                ast = JavaScriptCodeToXml.Instance.Generate(
                        code, p => p.program());
                var program = JavaScriptProgramGeneratorHelper.CreateProgram(
                        ast);

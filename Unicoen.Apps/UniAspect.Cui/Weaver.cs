    ...
    public static class Weaver {
        ...
        public static void Run(string directoryPath) {
            ...
            foreach (var file in targetFiles) {
                ...
                File.WriteAllText(
                        newPath,
                        UnifiedGenerators.GetCodeGeneratorByExtension(
                                Path.GetExtension(file)).Generate(model));
            }
        }
        ...
        public static void Weave(string language, UnifiedProgram model) {
            ...
            foreach (var intertype in _visitor.Intertypes) {
                ...
                InterType.AddIntertypeDeclaration(
                        model, intertype.GetTarget(), members);
            }
            ...
            foreach (var advice in _visitor.Advices) {
                ...
                case "before":
                    ...
                    CodeProcessorProvider.WeavingBefore(
                            target.GetPointcutType(), model, methodName, code);
                    break;
                case "after":

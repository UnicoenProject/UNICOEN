...
namespace Unicoen.Apps.RefactoringDSL.Tests.NamespaceDetector {
    public class NamespaceDetectorTest {
        ...
        public void パッケージ空間名を正しく取得できる() {
            var packageNode =
                    _model.FirstDescendant<UnifiedNamespaceDefinition>();
        ...
        public void クラス空間名を正しく取得できる() {
            var targetClass =
                    FindUtil.FindClassByClassName(_model, "Inner").First();
            var nsString =
                    Detector.GetNamespace(targetClass).GetNamespaceString();
            const string expected =
                    "pkg.subpkg.subsubpkg.subsubsubpkg.Cls.Inner";
            Assert.That(nsString, Is.EqualTo(expected));
        }
        ...
        public void 関数空間名を正しく取得できる() {
            var targetClass =
                    FindUtil.FindClassByClassName(_model, "Cls").First();
            var functionNode =
                    targetClass.FirstDescendant<UnifiedFunctionDefinition>();
            var nsString =
                    Detector.GetNamespace(functionNode).GetNamespaceString();
            const string expected =
                    "pkg.subpkg.subsubpkg.subsubsubpkg.Cls.method";
            Assert.That(nsString, Is.EqualTo(expected));
        ...
        public void 変数空間名を正しく取得できる() {
            var targetClass =
                    FindUtil.FindClassByClassName(_model, "Cls").First();
            var functionNode =
                    targetClass.FirstDescendant<UnifiedVariableDefinition>();
            var nsString =
                    Detector.GetNamespace(functionNode).GetNamespaceString();
            var detailedNsString =
                    Detector.GetNamespace(functionNode).
                            GetDetailedNamespaceString();
            Console.WriteLine(nsString);
        }
        ...
        public void While空間名を正しく取得できる() {
            ...
            var detailedNsString =
                    Detector.GetNamespace(whileNode).GetDetailedNamespaceString(
                            );
            Console.WriteLine(nsString);
            Console.WriteLine(detailedNsString);
        }
        ...
        public void 名前空間からコードオブジェクトを検索できる() {
            const string nsString =
                    "pkg.subpkg.subsubpkg.subsubsubpkg.Cls.method2.(for)";
            var found = Application.FindUnifiedElementByNamespace(
                    nsString, _model);
        ...
        public void 名前空間に対応する要素数は必ず1() {
            ...
            foreach (var vn in variableNodes) {
                var namespaceString =
                        Detector.GetNamespace(vn).GetNamespaceString();
                Assert.That(
                        Application.FindUnifiedElementByNamespace(
                                namespaceString, _model).Count(), Is.EqualTo(1));
            }
        }
    }
}
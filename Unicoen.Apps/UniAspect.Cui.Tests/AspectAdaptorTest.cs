...
namespace Unicoen.Apps.UniAspect.Cui {
    ...
    public class AspectAdaptorTest {
        ...
        public void AssertCorrectWeavingForJava(
                string aspectFile, string expectationFile) {
            //アスペクトモデルの作成
            ...
            var expectation = UnifiedGenerators.GenerateProgramFromFile(
                    filePath);
        ...
        /// <param name="aspectFile"> 入力アスペクトファイル名 </param>
        /// <param name="expectationFile"> アスペクト合成後の期待値ファイル名 </param>
        public void AssertCorrectWeavingForJavaScript(
                string aspectFile, string expectationFile) {
            //アスペクトモデルの作成
            var aspectPath = FixtureUtil.GetAspectPath(aspectFile);
            Weaver.AnalizeAspect(aspectPath);
        ...
        public void JavaScript言語にインタータイプ宣言が正しく合成される() {
            ...
            var amountOfMethodInJavaScript =
                    _javaScriptModel.Descendants<UnifiedFunctionDefinition>().
                            Count();
        ...
        public void JavaScript言語の関数呼び出し前にコードが正しく合成される() {
            AssertCorrectWeavingForJavaScript(
                    "before_call.apt", "before_call.js");
        }
        ...
        public void JavaScript言語の関数実行前にコードが正しく合成される() {
            AssertCorrectWeavingForJavaScript(
                    "before_execution.apt", "before_execution.js");
        }

        [Test]
        public void JavaScript言語の関数実行後にコードが正しく合成される() {
            AssertCorrectWeavingForJavaScript(
                    "after_execution.apt", "after_execution.js");
        }
        ...
        public void Java言語にインタータイプ宣言が正しく合成される() {
            ...
            //期待されるモデルの作成
            var filePath = FixtureUtil.GetAspectExpectationPath(
                    "intertype.java");
            var expectation = UnifiedGenerators.GenerateProgramFromFile(
                    filePath);
        ...
        public void Java言語の関数実行前にコードが正しく合成される() {
            AssertCorrectWeavingForJava(
                    "before_execution.apt", "before_execution.java");
        }
        ...
        public void Java言語の関数実行後にコードが正しく合成される() {
            AssertCorrectWeavingForJava(
                    "after_execution.apt", "after_execution.java");
        }
    }
}